...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`🔍 Looking up creator: "${cleanCreator}" (original: "${creator}")`)
...
Use exact tool names and proper parameters. No explanations, just JSON array.`
        ...
        console.log(`🤖 Using LLM to orchestrate MCP calls (attempt ${retryCount + 1})...`)
        ...
        if (retryCount >= maxRetries) {
          ...
          try {
            ...
            return await parseAndReturnCreatorData(directResult, cleanCreator, res, 'Direct MCP')
...
Extract the HIGHEST/MOST REALISTIC numbers you find.`
    ...
    try {
      ...
      return await parseAndReturnCreatorData({ content: [{ type: 'text', text: allResponseText }] }, cleanCreator, res, 'Regex Fallback')
    }
    ...
    console.log(`✅ LLM-extracted data for @${cleanCreator}: ${extractedData.followers.toLocaleString()} followers, ${extractedData.engagements.toLocaleString()} engagements`)
    ...
    // Fallback to regex parsing
    return await parseAndReturnCreatorData(mcpResults[0]?.result, cleanCreator, res, 'LLM Fallback')
  }
}
...
async function parseAndReturnCreatorData(result, cleanCreator, res, source) {
    ...
    for (const content of result.content) {
      if (content.type === 'text') {
        ...
        for (const match of followerMatches) {
          if (match) {
            followerCount = Math.max(followerCount, parseInt(match[1].replace(/,/g, '')))
          }
        }
  ...
  console.log(`✅ ${source} lookup found @${cleanCreator}: ${followerCount.toLocaleString()} followers, ${engagements.toLocaleString()} engagements`)
