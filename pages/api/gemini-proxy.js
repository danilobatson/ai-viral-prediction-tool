...
async function getGeminiMarketingAnalysis(postData, creatorData, trendingTopics) {
  try {
...
- Creator Rank: #${creatorData.creatorRank || creatorData.influencer_rank || 'Unknown'}

POST CONTENT TO ANALYZE:
"${postData.text}"

CURRENT TRENDING TOPICS:
${trendingTopics?.map(t => `- ${t.name} (${t.change > 0 ? '+' : ''}${t.change}%)`).join('\n') || '- No trending data available'}
...
Analyze like a true viral marketing expert with 10+ years of experience.`;
    ...
    console.log('🤖 Calling Google Gemini 2.0 Flash Lite for AI marketing analysis...');
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          expectedReach: analysisData.expectedReach || Math.round(creatorData.followers * 0.1),
          explanation: analysisData.explanation || text,
          recommendations: analysisData.recommendations || [
            { title: 'AI Analysis', description: 'Detailed analysis completed' }
          ],
          keyFactors: analysisData.keyFactors || ['Content quality', 'Creator influence'],
      ...
      console.warn('⚠️ Could not parse Gemini JSON response, using text analysis');
    }
    ...
    return {
      ...
      explanation: text,
      recommendations: [
        { title: 'AI Analysis', description: 'Complete analysis provided by Gemini AI' }
      ],
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (prompt) {
      ...
      result = {
        success: true,
        analysis: await getGeminiMarketingAnalysis(postData, creatorData, trendingTopics),
